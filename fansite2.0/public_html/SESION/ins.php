...
if (isset($_POST['btnins'])) {
    ...
    if ($user && password_verify($pass, $user['passw'])) {
        ...
        if ($user['user_type'] == 'user') {
            echo "<script>alert('Bienvenido: " . $_SESSION['correo'] . "'); window.location='../CARRITO2/Carrito.php'</script>";
        } else {
            echo "<script>alert('Bienvenido Administrador: " . $_SESSION['correo'] . "'); window.location='../CARRITO2/Carrito.php'</script>";
        }
    } else {
